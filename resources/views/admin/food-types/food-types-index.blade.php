...
@section('main')
    <!-- Heading -->
    <h1 class="mb-3 text-xl font-semibold dark:text-white">Food Types</h1>

    <div class="lg:flex lg:justify-between items-center">
        <!-- Search Bar -->
        <x-web.table-search-bar :url="route('admin.food-types.index')" />
        ...
        <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
            ...
            @foreach($foodTypes as $foodType)
                <tr class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <td class="px-6 py-4">
                        @if(count($foodType->media) > 0)
                            <img src="{{ $foodType->media[0]->getUrl() }}" class="!max-w-none h-20 w-20 object-fit">
                        @endif
                    </td>
                    <td class="px-6 py-4">
                        {{ $foodType->name }}
                    ...
                    <td class="px-6 py-4">
                        {{ $foodType->created_at->diffForHumans() }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $foodType->updated_at->diffForHumans() }}
                    </td>
                    <td class="px-6 py-4">
                        <a href="{{ route('admin.food-types.edit', $foodType->id) }}"
                           class="mr-2 text-blue-500 hover:underline">Edit</a>
                        <form action="{{ route('admin.food-types.destroy',$foodType->id) }}" method="Post"
