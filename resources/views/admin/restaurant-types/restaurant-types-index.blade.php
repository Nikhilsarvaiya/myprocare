...
@section('main')
    <!-- Heading -->
    <h1 class="mb-3 text-xl font-semibold dark:text-white">Restaurant Types</h1>

    <div class="lg:flex lg:justify-between items-center">
        <!-- Search Bar -->
        <x-web.table-search-bar :url="route('admin.restaurant-types.index')" />
        ...
        <div class="mb-3 flex items-center space-x-3">
            ...
            <a href="{{ route('admin.restaurant-types.create') }}">
                <x-secondary-button>
                    Add
                </x-secondary-button>
            </a>
        ...
        <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
            ...
            @foreach($restaurantTypes as $restaurantType)
                <tr class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <td class="px-6 py-4">
                        @if(count($restaurantType->media) > 0)
                            <img src="{{ $restaurantType->media[0]->getUrl() }}" class="!max-w-none h-20 w-20 object-fit">
                        @endif
                    </td>
                    <td class="px-6 py-4">
                        {{ $restaurantType->name }}
                    ...
                    <td class="px-6 py-4">
                        {{ $restaurantType->created_at->diffForHumans() }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $restaurantType->updated_at->diffForHumans() }}
                    </td>
                    <td class="px-6 py-4">
                        <a href="{{ route('admin.restaurant-types.edit', $restaurantType->id) }}"
                           class="mr-2 text-blue-500 hover:underline">Edit</a>
                        <form action="{{ route('admin.restaurant-types.destroy',$restaurantType->id) }}" method="Post"
