...
@section('main')
    ...
    <div class="flex justify-between">
        ...
        @php
         if (request()->route('redirect') === "edit"){
             $backUrl = route('user.businesses.products.edit.index', request()->route('business'));
         }else{
             $backUrl = route('user.businesses.products.create.index', request()->route('business'));
         }
        @endphp
        <a href="{{ $backUrl }}">
            <x-secondary-button>
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ route('user.businesses.products.update', ['business' => request()->route('business'), 'product' => $product->id, 'redirect' => request('redirect')]) }}">
        @csrf
        @method('PUT')
