...
@section('main')
    <div class="lg:flex lg:justify-between items-center">
        <x-web.page-title>Wallet ({{ $wallet->balance ?? 0 }} RP)</x-web.page-title>

        <div class="mb-3 flex items-center space-x-3">
            <a href="{{ route('user.wallet-transactions.scan', ['type' => 'send']) }}">
                <x-secondary-button>
                    Send
                </x-secondary-button>
            </a>
    ...
    <x-table.main :columns="$columns">
        @foreach($wallet_transactions as $wallet_transaction)
            <x-table.tr>
                ...
                <x-table.td>{{ $wallet_transaction->sender->name }}</x-table.td>
