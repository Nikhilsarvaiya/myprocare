...
@section('main')
    <x-web.page-title :title="'Deals'"/>

    <div class="lg:flex lg:justify-between items-center">
        <!-- Search Bar -->
        <x-web.table-search-bar :url="route('user.deals.index')" />
        ...
        <div class="mb-3 flex items-center space-x-3">
            ...
            <a href="{{ route('user.deals.create') }}">
    ...
    <div class="relative overflow-x-auto shadow-md rounded-lg">
        <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
            ...
            <tr class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
                ...
                <td class="px-6 py-4">
                    @if(count($deal->media) > 0)
                        <img src="{{ $deal->media[0]->getUrl() }}" class="!max-w-none h-20 w-20 object-fit">
                    @endif
                </td>
                <td class="px-6 py-4">
                    {{ $deal->name }}
                ...
                <td class="px-6 py-4">
                    <a href="{{ route('user.deals.edit', $deal->id) }}" class="mr-2 text-blue-500 hover:underline">Edit</a>
                    <form action="{{ route('user.deals.destroy',$deal->id) }}" method="Post"
                          onclick="return confirm('Are you sure you want to delete deal?')">
                        @csrf
                        @method('DELETE')
