...
class BusinessProductController extends Controller
{
    ...
    public function createIndex(Business $business)
    {
        ...
        return view('admin.businesses.businesses-products-create-index', compact('products'));
    }

    public function editIndex(Business $business){
        $products = $this->businessProductService->getProducts($business->id);

        return view('admin.businesses.businesses-products-edit-index', compact('products'));
    }
    ...
    public function store(StoreProductRequest $request, Business $business)
    {
        ...
        if ($request->isMethod('post')){
            return redirect()->route('admin.businesses.products.create.index', $business->id);
        }else{
            // put method
    ...
    public function edit(Request $request, $businessId, Product $product){
        return view('admin.businesses.businesses-products-edit', compact('product'));
    }
    ...
    public function update(UpdateProductRequest $request, $businessId, Product $product){
        ...
        if ($request->redirect === "edit"){
            return redirect()->route('admin.businesses.products.edit.index', $businessId);
        }else{
            return redirect()->route('admin.businesses.products.create.index', $businessId);
        }
    }
